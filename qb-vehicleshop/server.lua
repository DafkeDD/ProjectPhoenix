...
AddEventHandler('qb-vehicleshop:client:orderVehicle', function(vehicle, data)
    local src = source
    local player = source
    local closestVehicle = GetClosestVehicle(player)
    local vehicle = vehicle.buyVehicle
    local vehiclePrice = QBCore.Shared.Vehicles[vehicle]['price']
    local price = round(vehiclePrice * Config.discount)
    local webhookUrl = Config.WebhookUrl
    local Player = QBCore.Functions.GetPlayer(src)
    local firstname = Player.PlayerData.charinfo.firstname
    local lastname = Player.PlayerData.charinfo.lastname
    local PhoneNumber = Player.PlayerData.charinfo.phone
    local vehicleObject = QBCore.Shared.Vehicles[vehicle]
    local vehicleCat = vehicleObject['category']
    local vehicleName = vehicleObject['name']
    local headers = { ['Content-Type'] = 'application/json' }
    local data = {
        ['content'] = 'A new vehicle has been ordered!',
        ['embeds'] = {
            {
                ['title'] = 'New Vehicle Order Has Been Placed',
    ...
    PerformHttpRequest(webhookUrl, function(statusCode, response, headers)
        TriggerClientEvent('QBCore:Notify', src, 'Your Order Has Been Placed', "success")
...
RegisterServerEvent('qb-vehicleshop:server:buy-stock', function(vehicle)
    vehicle = vehicle.buyVehicle
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local steamname = GetPlayerName(src)
    local vehiclePrice = QBCore.Shared.Vehicles[vehicle]['price']
    local discount = round(vehiclePrice * Config.discount)
    local account = exports["qb-management"]:GetAccount(Player.PlayerData.job.name)
    if account >= discount  then
        TriggerClientEvent("qb-vehicleshop:client:create_blip", src, vehicle)
        exports["qb-management"]:RemoveMoney(tostring(Player.PlayerData.job.name), discount)
        TriggerClientEvent('QBCore:Notify', src, 'Vehicle Successfully Bought', "success")
    else
        TriggerClientEvent('QBCore:Notify', src, 'The Dealership does not have enough money', "error")
    end
end)

RegisterServerEvent('qb-vehicleshop:owncar', function(k, vehicle)
    local stock = nil
    local car = vehicle
    local isthere = false
    local car = MySQL.query.await('SELECT * FROM vehicle_stock WHERE car = ?', { vehicle })
...
RegisterNetEvent('qb-vehicleshop:server:updatestock', function(data)
...
RegisterNetEvent('qb-vehicleshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle, stock)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    if stock < 1 then
        TriggerClientEvent('QBCore:Notify', src, 'Out of stock', 'error')
    ...
    if paymentAmount > Config.MaximumPayments then return TriggerClientEvent('QBCore:Notify', src,
            Lang:t('error.exceededmax'), 'error')
    end
    local plate = GeneratePlate()
